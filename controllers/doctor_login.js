...
router.post('/',[
    ...
    if (username && password){
        con.query('select * from doctor where username = ? and password = ?' , [username, password], function(error , results , fields){
            if (results.length > 0){
                
                request.session.loggedin = true ; 
                request.session.username = username;
                request.session.d_id = results[0].d_id;
              ...
              //  }
              //  else{
                    sweetalert.fire('Doctor logged In!');
